const tabelEl = document.getElementById("penerbit");
...
document.addEventListener("click", (e) => {
  ...
  if (e.target.id == "editBtn") {
    window.location.href =
      "/PerpusJava/admin/pages/forms/editpenerbit.html?id=" + kode;
  }
  if (e.target.id == "deleteBtn") {
    Swal.fire({
      title: "Apakah anda yakin?",
      text: `Anda akan menghapus data penerbit dengan id ${kode}`,
      icon: "warning",
        ...
        fetch(
          ...
          .then((data) => {
            if (data.status === "OK") {
              ...
              Swal.fire(
                "Dihapus!",
                `Penerbit dengan id ${kode} telah dihapus`,
                "success"
              );
            }
          });
...
function showAllDatas() {
  ...
  getAllDatas().then((datas) => {
    datas.forEach((data) => {
      ...
      const editBtn = makeEditBtn(data.idpenerbit);

      // Define delete button element
      const removeBtn = makeDeleteBtn(data.idpenerbit);
...
async function getAllDatas() {
  const res = await fetch("/PerpusJava/PenerbitController");
...
getAllDatas().then((datas) => {
  ...
  function changePage(page) {
    ...
    for (
      ...
      // Define edit button element
      const editBtn = makeEditBtn(buku.idpenerbit);

      // Define delete button element
      const removeBtn = makeDeleteBtn(buku.idpenerbit);
