const tabelEl = document.getElementById("petugas");
...
document.addEventListener("click", (e) => {
  ...
  if (e.target.id == "editBtn") {
    window.location.href =
      "/PerpusJava/admin/pages/forms/editpetugas.html?id=" + kode;
  }
  if (e.target.id == "deleteBtn") {
    Swal.fire({
      title: "Apakah anda yakin?",
      text: `Anda akan menghapus data petugas dengan id ${kode}`,
      icon: "warning",
        ...
        fetch(
          ...
          .then((data) => {
            if (data.status === "OK") {
              ...
              Swal.fire(
                "Dihapus!",
                `Petugas dengan id ${kode} telah dihapus`,
                "success"
              );
            }
          });
...
function showAllDatas() {
  ...
  getAllDatas().then((datas) => {
    datas.forEach((data) => {
      ...
      const editBtn = makeEditBtn(data.idpetugas);

      // Define delete button element
      const removeBtn = makeDeleteBtn(data.idpetugas);
...
getAllDatas().then((datas) => {
  ...
  function changePage(page) {
    ...
    for (
      ...
      // Define edit button element
      const editBtn = makeEditBtn(buku.idpetugas);

      // Define delete button element
      const removeBtn = makeDeleteBtn(buku.idpetugas);
